...
def divide_file_name(name):
    ...
    while(i < len(name)):
        ...
        i += 1
    return before, after
def parse_all_in_one(input_path, output_path):
    """解析PDF文本，并保存到TXT文件中"""
        ...
        for page in doc.get_pages():
            ...
            for x in layout:
                if(isinstance(x, LTTextBoxHorizontal)):
                    with open(output_path+"/all_in_one.txt", 'a', encoding='utf-8') as f:
                        results = x.get_text()
                       # return results
                        f.write(results+'\n')
...
print("failed num: %d" % len(fail_list))

# convert to one big txt
is_all_in_one = 1
fail_list = []
total_num = len(pdfs)
i = 0
